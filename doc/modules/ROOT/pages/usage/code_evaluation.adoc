...
pretty simple: legacy. The principal sources of inspiration for CIDER--Emacs
and SLIME--provide more or less the same functionality, but use different
keybindings. CIDER tried to find a common ground by adopting them both.

On top of this, when it became clear that CIDER had set the world record
for number of evaluation commands, we introduced a dedicated keymap for all eval
commands (that's everything with the prefix kbd:[C-c C-v]). This leads to funny
situations like `cider-eval-defun-at-point` having 3 keybindings:
